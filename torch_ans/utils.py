...
def pmf_to_quantized_cdf_batched(pmf: torch.Tensor, precision=16, add_tail=True, tail_mass=1e-10, normalize=True):
    ...
    cdf_float = torch.cat(
        [
            ...
            cdf_float[:, :-1],
            torch.zeros(pmf.shape[0], 1).type_as(cdf_float) + max_index,
        ],
        dim=1,
    )
    cdf = cdf_float.int()
...
class TorchEntropyCoderBaseInterface(object):
    ...
    def encode_with_indexes(self, symbols: torch.Tensor, indexes: torch.Tensor, cache=False, **kwargs) -> bytes:
        ...
        if cache:
            if len(self.cache_encode_with_indexes) == 0:
                self.cache_encode_with_indexes = dict(
                    symbols=self._init_tensor(symbols), indexes=self._init_tensor(indexes), **kwargs
                )
            else:
                self.cache_encode_with_indexes['symbols'] = self._cache_concat(
                    self.cache_encode_with_indexes['symbols'], self._init_tensor(symbols)
                )
                self.cache_encode_with_indexes['indexes'] = self._cache_concat(
                    self.cache_encode_with_indexes['indexes'], self._init_tensor(indexes)
                )
        else:
    ...
    def encode_with_freqs(
        self,
        symbols: torch.Tensor,
        freqs: Optional[torch.Tensor],
        num_freqs: Optional[torch.Tensor],
        cache=False,
        **kwargs,
    ) -> bytes:
        """
        ...
        if cache:
            if len(self.cache_encode_with_freqs) == 0:
                ...
                self.cache_encode_with_freqs['symbols'] = self._cache_concat(
                    self.cache_encode_with_freqs['symbols'], self._init_tensor(symbols)
                )
                self.cache_encode_with_freqs['freqs'] = self._cache_concat(
                    self.cache_encode_with_freqs['freqs'], self._init_tensor(freqs)
                )
                self.cache_encode_with_freqs['num_freqs'] = self._cache_concat(
                    self.cache_encode_with_freqs['num_freqs'], self._init_tensor(num_freqs)
                )
        else:
            raise NotImplementedError()
    ...
    def encode_symbols(self, symbols: torch.Tensor, embedded_freqs=False, cache=False, **kwargs) -> bytes:
        ...
        if cache:
            if len(self.cache_encode_symbols) == 0:
                self.cache_encode_symbols = dict(
                    symbols=self._init_tensor(symbols), embedded_freqs=embedded_freqs, **kwargs
                )
            else:
                self.cache_encode_symbols['symbols'] = self._cache_concat(
                    self.cache_encode_symbols['symbols'], self._init_tensor(symbols)
                )
        else:
            raise NotImplementedError()
    ...
    def decode_with_freqs(
        self, encoded: str, freqs: Optional[torch.Tensor], num_freqs: Optional[torch.Tensor], **kwargs
    ) -> torch.Tensor:
        """
...
class TorchANSInterface(TorchEntropyCoderBaseInterface):
    ...
    def encode_with_indexes(self, symbols: torch.Tensor, indexes: torch.Tensor, cache=False, **kwargs) -> bytes:
        ...
        if cache:
            ...
            stream = self._init_tensor(
                self.ans_init_func(num_parallel_states)
            )  # , device=self.device)) # TODO: create stream on device
            # if self.device is not None:
            #     stream = stream.to(device=self.device)
            #     symbols = symbols.to(device=self.device)
            #     indexes = indexes.to(device=self.device)

            self.ans_encode_func(
                stream,
            ...
            return merge_bytes(
                byte_strings, num_bytes_length=self.num_bytes_code_length, num_segments=self.num_parallel_states
            )
    ...
    def decode_with_indexes(self, encoded: str, indexes: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Decoded symbols.
        """
        byte_strings = split_merged_bytes(
            encoded, num_bytes_length=self.num_bytes_code_length, num_segments=self.num_parallel_states
        )
